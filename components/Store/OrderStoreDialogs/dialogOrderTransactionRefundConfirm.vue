<template>
  <dialog-common v-model="value" :title="$t('onlineOrder.refundDialog.confirmation')">
    ...
    <div class="mt-2" style="background-color: #eee; padding: 5px; border-radius: 5px;">
      ...
      <div>
        <div>ID: #{{ order.id }}</div>
        <div>{{ $t('onlineOrder.refundDialog.totalAmount') }}:
          {{ $t('common.currency', storeLocale) }}{{ order.payment[0].value }}
        </div>
        <div>{{ $t('onlineOrder.refundDialog.orderTime') }}: {{ formatDate(order.date) }}</div>
    ...
    <template #actions>
      ...
      <g-btn-bs @click="submit" background-color="#FF5252" text-color="white" width="100" style="margin: 0">
        {{ $t('onlineOrder.refundDialog.ok') }}
      </g-btn-bs>
    </template>
  </dialog-common>
</template>
