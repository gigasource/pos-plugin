<template>
  <g-btn :uppercase="false" background-color="white" text-color="#1d1d26" class="mr-3" :disabled="!selectedTaxCategory"
         @click="openDialogEditTaxCategory">
    <g-icon class="mr-2" color="red">
      edit
    </g-icon>
    {{$t('ui.edit')}}
  </g-btn>
  <g-btn :uppercase="false" class="mr-3" background-color="white" :disabled="!selectedTaxCategory" text-color="#1d1d26"
  ...
  <dialog-confirm-delete type="tax category" :label="selectedTaxCategory ? selectedTaxCategory.value + '%' : ''"
                         v-model="dialogConfirmDelete" @submit="deleteTaxCategory"/>
  <dialog-new-tax-category ref="dialog"/>
</template>
