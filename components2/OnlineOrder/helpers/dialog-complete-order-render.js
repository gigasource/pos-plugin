		...
		return (
			<div className="row-flex mb-2">
				<div style="flex: 0 0 30px">
					{
						(dialogOrder.value.type === 'delivery') &&
						<g-icon>
							icon-delivery-scooter </g-icon>
					}
					{
						(dialogOrder.value.type === 'pickup') &&
						<g-icon>
							icon-take-away </g-icon>
					}
				</div>
				<p>
          <span className="text-indigo-accent-2 fw-600">
            #{dialogOrder.value.dailyId ? dialogOrder.value.dailyId : dialogOrder.value.id}
				...
				<g-spacer></g-spacer>
				<span className="fw-700 fs-large">
	...
	const renderContentAddress = function () {
		return (dialogOrder.value.type === 'delivery') &&
			<div class="row-flex mb-2">
				...
				<div class="col-10">
					{`${dialogOrder.value.customer.address} ${dialogOrder.value.customer.zipCode}`} </div>
			</div>
	}
	...
	const renderForwadedStore = function () {
		return (dialogOrder.value.forwardedStore) &&
			<g-card-actions class="forward-store">
				<b class="mr-1">
					From: </b>
				{dialogOrder.value.forwardedStore}
			</g-card-actions>
	}
		...
		return <>
			{
				...
				<g-dialog v-model={completeOrderDialogShow.value} width="580px">
					<g-card className="px-3 pb-2">
						...
						<g-card-text className="fs-small">
							{renderContentInfo()}
							...
							<div className="dashed-gradient mt-2"></div>
							<div className="row-flex justify-between mt-2"
							...
							<div className="row-flex justify-between mt-1"
								...
								<div className="ta-right row-flex align-items-center" style="text-transform: capitalize">
									{
										(paymentMethod.value.icon) &&
										<img src={paymentMethod.value.icon} style="height: 16px" className="mr-1"> </img>
									}
									<span>
