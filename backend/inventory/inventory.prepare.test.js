...
async function prepareInventoryRetailDb(orm) {
	const InventoryRetail = orm('InventoryRetail')
	const InventoryRetailCategory = orm('InventoryRetailCategory')
	const InventoryRetailHistory = orm('InventoryRetailHistory')
	...
	// inventories = inventories.map(async inventory => {
	// 	return await makeInventoryRetail(inventory)
	// })

	for (let i in inventories) {
		inventories[i] = await makeInventoryRetail(inventories[i])
	}
	...
	for (let combo of combos) {
		inventories.push(await makeInventoryRetail(combo))
	}
}
