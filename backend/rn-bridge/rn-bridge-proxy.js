...
let clientSocket = {
	emit: function () {
		...
		for (let i = 1; i < arguments.length; i++) {
			mess.push(arguments[i]);
		}
		queueMess.push(mess);
	}
};
socket = cms.io.of('/proxyAppRn');
socket.on('connect', socket => {
	clientSocket = socket;
	console.log('React connected');
	socket.emit('message', SYSTEM_CHANNEL, 'ready-for-app-events');
	for (let mess of queueMess) {
		socket.emit('message', ...mess);
	}
	socket.on('message', (channel, data) => {
		bridgeListener(channel, data);
	})
})
...
class EventChannel extends ChannelSuper {
	post(event, ...msg) {
		...
		clientSocket.emit('message', this.name, MessageCodec.serialize(event, ...msg));
	};
...
class SystemChannel extends ChannelSuper {
		...
		if (event.startsWith('pause')) {
			setImmediate( () => {
				...
				let eventLock = new SystemEventLock(
					() => {
						clientSocket.emit('message', _this.name, releaseMessage);
	...
	post(event, ...msg) {
		...
		clientSocket.emit('message', SYSTEM_CHANNEL, MessageCodec.serialize(event, ...msg));
	}
	...
	sendObject(event, object) {
		...
		clientSocket.emit('message', this.name, JSON.stringify({event, payload: object}));
	}
};
/**
