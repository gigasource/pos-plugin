...
let NativeBridge
try {
  NativeBridge = process._linkedBinding('node_bridge');
} catch (err) {
  console.error("Can not find node_bridge module");
}

if (!NativeBridge) {
  NativeBridge = {
    sendMessage: function(...args) {},
    registerChannel: function (...args) {}
  }
}
...
NativeBridge.sendMessage(SYSTEM_CHANNEL, "Node Ready");

module.exports = async cms => {
  cms.nodeRun = () => {
    console.log("Sending ready signal");
    NativeBridge.sendMessage(SYSTEM_CHANNEL, "Start Node");
  }
  return {
    app: systemChannel,
